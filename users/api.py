...
class UsersView(APIView):
    ...
    def get(self, request):
        try:
            ...
            if role == 'admin' or (role == 'super_admin' and scope == 'tenant'):
                try:
                ...
                if admin_tenant:
                    users = User.objects.filter(profile__role='employee', profile__tenant=admin_tenant, is_active=True).select_related('profile')
                else:
